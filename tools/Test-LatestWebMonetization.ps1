...
Test-Condition ($index.Contains('game.js?v=2026-08-31-build-15') -and $index.IndexOf('id="monetizationActions"') -lt $index.IndexOf('class="mode-grid"')) 'TestFlight loads build 15 purchase diagnostics and places monetization before the game modes'
...
Test-Condition (([regex]::Matches($project, 'CURRENT_PROJECT_VERSION = 15;')).Count -eq 2 -and ([regex]::Matches($project, 'MARKETING_VERSION = 3\.0\.0;')).Count -eq 2) 'Debug and Release identify App Store version 3.0.0 build 15'
Test-Condition ($purchase.Contains('startupProductRetryDelaysNanoseconds') -and $purchase.Contains('purchaseProductRetryDelaysNanoseconds') -and $purchase.Contains('loadProductWithRetry') -and $purchase.Contains('Storefront.updates') -and $purchase.Contains('Storefront.current')) 'StoreKit retries product loading and refreshes when the active storefront changes'
...
Test-Condition ($swiftBridge.Contains('"purchaseCatalogState"') -and $swiftBridge.Contains('"purchaseDiagnosticCode"') -and $purchase.Contains('"catalog_empty"')) 'Build 15 exposes bounded StoreKit catalog diagnostics to TestFlight'
