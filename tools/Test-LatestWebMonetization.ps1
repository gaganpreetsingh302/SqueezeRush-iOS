...
Test-Condition ($index.Contains('game.js?v=2026-08-31-build-17') -and $index.IndexOf('id="monetizationActions"') -lt $index.IndexOf('class="mode-grid"')) 'TestFlight loads build 17 purchase diagnostics and places monetization before the game modes'
...
Test-Condition (([regex]::Matches($project, 'CURRENT_PROJECT_VERSION = 17;')).Count -eq 2 -and ([regex]::Matches($project, 'MARKETING_VERSION = 3\.0\.0;')).Count -eq 2) 'Debug and Release identify App Store version 3.0.0 build 17'
