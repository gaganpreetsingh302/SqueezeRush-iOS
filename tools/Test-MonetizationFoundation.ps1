...
Test-Condition ($bridge.Contains('"removeAdsEntitled"') -and $bridge.Contains('"removeAdsPrice"') -and $bridge.Contains('"removeAdsCurrencyCode"')) 'iOS capabilities expose bounded Remove Ads entitlement, localized price, and currency data'
Test-Condition ($purchase.Contains('priceFormatStyle.currencyCode.uppercased()')) 'iOS derives the displayed ISO currency code from StoreKit product pricing metadata'
...
Test-Condition ($purchase.Contains('startupProductRetryDelaysNanoseconds') -and $purchase.Contains('purchaseProductRetryDelaysNanoseconds') -and $purchase.Contains('loadProductWithRetry') -and $purchase.Contains('Storefront.updates') -and $purchase.Contains('refreshStorefrontDiagnostics()')) 'iOS retries StoreKit product loading and refreshes storefront diagnostics independently'
...
Test-Condition ($purchase.Contains('productRequestTimeoutNanoseconds') -and $purchase.Contains('productsWithTimeout(for:') -and $purchase.Contains('"catalog_timeout"')) 'iOS bounds StoreKit product requests with an explicit timeout'
...
Test-Condition ($bridge.Contains('import CoreFoundation') -and $bridge.Contains('CFGetTypeID(number) != CFBooleanGetTypeID()') -and -not $bridge.Contains('if value is Bool')) 'iOS distinguishes JavaScript protocol numbers from JSON booleans'
